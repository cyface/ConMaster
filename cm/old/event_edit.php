...
$tpl = new TemplatePower('./event_edit.tpl'); //make a new TemplatePower object

$tpl->prepare();//let TemplatePower do its thing, parsing etc.

$event = new DBObject('event'); //Create a new object to hold the values and execute the save.

if (empty($_GET['id']) & !empty($_POST['id'])) { //Trying to Save
	if ($_POST['rpga'] == 'CHECKED') {
	    $rpga_msg = 'RPGA ';
	} else {
		$rpga_msg = '';
	}
	$_POST['event_name'] = $rpga_msg . $_POST['game_system'] . ': ' . $_POST['scenario_name'];

	if ($event->update_all($_POST)) { //Try to save the values to the DB using the object
		$tpl->assign('action_message', '<font color="#66CC00">Saved</font>');
		valueFill($event->get_data(),$tpl);
		...
		errorFill($event,$tpl); //Fill in the error message placeholders with the save errors
	}
} else if (!empty($_GET['id'])) { //Display the edit form for the supplied
	$event->load_all($_GET['id']);
	valueFill($event->get_data(),$tpl);
} else {
...
unset($event); //Destroy the object
?>
